const fetchAllRecords = async (setRequests, setCurrentRequest, setIndexCounter) => {
    const allRequestsRequest = await fetch('https://squid-app-i93aj.ondigitalocean.app/api/all-requests', {
        method: 'GET',
        credentials: 'include',
    });
...
const fetchMyRecords = async (setRequests, setCurrentRequest, setIndexCounter, setFilter) => {
    const myRequestsRequest = await fetch('https://squid-app-i93aj.ondigitalocean.app/api/my-requests', {
        method: 'GET', 
        credentials: 'include',
    });
...
const fetchStatusRecord = async (setRequests, setCurrentRequest, setIndexCounter, statusFilter, setFilter) => {
    const statusRequestsRequest = await fetch(`https://squid-app-i93aj.ondigitalocean.app/api/status-requests/${JSON.stringify(statusFilter)}`, {
        method: 'GET', 
        credentials: 'include',
    });
...
const updateRecordStatus = async (record, newStatus, setRequests, setCurrentRequest) => {
    const statusUpdateRequestRequest = await fetch(`https://squid-app-i93aj.ondigitalocean.app/api/update-request-status`, {
        method: 'PUT', 
        credentials: 'include',
        headers: {
...
const checkOwnership =  async (record, setIsOwner) => {
    ...
    const ownershipRequest = await fetch(`https://squid-app-i93aj.ondigitalocean.app/api/check-ownership/${JSON.stringify(payload)}`, {
        method: 'GET',
        credentials: 'include',
    });
...
const deleteRecord = async (record, setDeleteIndicator, btn) => {
    const deleteRequest = await fetch(`https://squid-app-i93aj.ondigitalocean.app/api/delete-record`, {
        method: 'DELETE',
        credentials: 'include',
        headers: {
            'content-type': 'application/json',
